...
CAUTION: 如果新舊控制器沒有通用連接埠，您就不需要修改資料生命期。跳過此步驟並直接前往<<upgrades_manual_without_matching_ports,步驟4.>>。
...
. [[vimization_manual ， no_matching_ports ]] 修改廣播網域以移除需要刪除的 VLAN 和實體連接埠：
+
...
.關於這項工作
* 您需要在中的升級程序稍早識別的 UUID 和系統 ID <<gather_info_so_sb,在升級之前收集資訊>>。
