...
* 下表列出了所有使用共享的 MetroCluster/HA 連接埠和共享的叢集/HA 連接埠的平台型號。並非所有組合都受此程序支援。請參閱 <<expand-ip-supported-platforms,支援的平台組合>> 以確認您的擴充組合是否受支援。
...
image:../media/expand-ip-group-1-updated.png["MetroCluster IP 中AFF和FAS組 1 平台的擴展組合"]
...
image:../media/expand-ip-group-2-updated.png["MetroCluster IP 中AFF和FAS組 2 平台的擴展組合"]
...
. 在維護模式下，發出 halt 命令退出維護模式：
+
《停止》

. 檢查 partner-sysid 是否正確：
+
《prontenv合作夥伴sysid》
...
+
有關完整的命令語法，請參閱 link:https://docs.netapp.com/us-en/ontap-cli["ONTAP 指令參考"^]。
...
+
以下範例將連接埠 "e0e" 和 "e0f" 指派給 SVM "cluster01" 上的容錯移轉群組 "intercluster01"：
...
+
以下範例顯示連接埠 "e0e" 上的叢集間 LIF "cluster01_icl01" 和 "cluster01_icl02" 將容錯移轉至連接埠 "e0f"。
...
== 建立 MetroCluster 介面並鏡像根聚合

您必須在新 MetroCluster IP 節點上建立 MetroCluster 介面並鏡像根 Aggregate。
...
NOTE: 此 `metrocluster configuration-settings connection show`命令的輸出結果可能因平台型號而異。例如，在某些平台上，HA Partner 的相關行不會顯示在輸出結果中。
...
[NOTE]
====
** 每個站台至少必須建立一個鏡射資料Aggregate。建議MetroCluster 每個站台在支援每個站台的兩個鏡射資料集合體、以裝載MDV磁碟區、不過每個站台只支援一個集合體（但不建議）。MetroCluster 的一個站台只有一個鏡射的資料集合體、另一個站台則有多個鏡射的資料集合體、這是可以接受的。
