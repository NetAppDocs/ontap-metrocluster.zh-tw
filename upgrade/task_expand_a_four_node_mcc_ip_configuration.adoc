...
.重要資訊
在過渡、刷新、八節點安裝和四節點擴充過程中，您可能會為 MetroCluster 配置中新增新的平台模型。在這些流程的任何階段，如果現有 MetroCluster 配置包含使用 *共用叢集/HA 連接埠* 的平台，則您必須將 ONTAP 版本升級至 ONTAP 9.15.1P11 或 ONTAP 9.16.1P4 或更高版本，才能將使用 *共用 MetroCluster/HA 連接埠* 的平台新增至設定中。
