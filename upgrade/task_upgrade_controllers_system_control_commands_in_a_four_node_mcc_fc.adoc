...
---
= 在四節點 MetroCluster FC 組態中，使用系統控制器替換來升級控制器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
從 ONTAP 9.10.1 開始，您可以使用此引導式自動 MetroCluster 切換程序，使用 `system controller replace` 命令對四節點 MetroCluster FC 組態執行不中斷營運的控制器升級。其他元件（例如儲存設備磁碟櫃或交換器）無法作為此程序的一部分進行升級。
...
如需其他程序，請參閱link:concept_choosing_an_upgrade_method_mcc.html["選擇升級或重新整理方法"]。
...
* 您可以使用此流程在四節點 MetroCluster FC 組態中使用基於 NSO 的自動切換和切換功能來升級控制器。如果您想使用彙總重定位 (ARL) 執行控制器升級，請參閱link:https://docs.netapp.com/us-en/ontap-systems-upgrade/upgrade-arl-auto-app/["使用「系統控制器更換」命令來升級執行ONTAP 效能不正常的控制器硬體"^]。建議使用基於 NSO 的自動化流程。
...
舊的系統 ID 在中識別 link:task_upgrade_controllers_system_control_commands_in_a_four_node_mcc_fc.html#gather-information-before-the-upgrade["在升級之前收集資訊"]。
...
.步驟
. 重複步驟以升級叢集 A 上的節點，從開始<<fc_controller_replace_prepare,準備升級>>。
+
