...
CAUTION: *每次升級控制器時，都必須按照說明執行此程序。 *當新平台發佈時，可能會有新的或修改後的步驟需要遵循。例如，當升級到ONTAP 9.15.1 或更高版本中引入的平台時，您必須 link:upgrade-mcc-ip-manual-set-bootarg.html["設定所需的啟動參數"] 並執行其他附加步驟以確保升級成功。
