...
---
= 在 MetroCluster IP 組態中，使用切換與切回將 AFF A700 升級至 AFF A900 或將 FAS9000 升級至 FAS9500
:allow-uri-read: 


[role="lead"]
從 ONTAP 9.10.1 開始，您可以使用 MetroCluster 切換作業，在升級合作夥伴叢集上的控制器模組時，為用戶端提供不中斷營運的服務。其他元件（例如儲存設備機櫃或交換器）無法做為此程序的一部分進行升級。
...
image::../media/workflow_ip_upgrade.png[MetroCluster IP 配置中升級控制器的工作流程]
...
** link:../install-ip/task_switch_config_cisco.html#reset-the-cisco-ip-switch-to-factory-defaults["將 Cisco IP 交換器重設為原廠預設值"]
...
** link:../install-ip/task_switch_config_cisco.html#download-and-install-the-cisco-ip-rcf-files["下載並安裝 Cisco IP RCF 檔案"]
...
. 如果 MetroCluster 節點配置了用於管理 SVM（叢集）的外部金鑰管理器，請將金鑰伺服器資訊複製到安全位置。如果配置了 Onboard Key Manager，請將備份資訊和密碼短語複製到安全位置。
+
[role="tabbed-block"]
====
.內建金鑰管理程式
--
.. 執行以下命令以顯示 Onboard Key Manager 備份：
+
「安全金鑰管理程式內建show Backup」

+
在升級程序的後續步驟中，您還需要最新的 Onboard Key Manager 密碼。
...
cluster_B
            admin   -          -          -           -          -
----
.. 取得與管理 SVM （叢集）上設定的外部金鑰管理程式相關聯的主要金鑰伺服器位址和連接埠號碼：
+
[NOTE]
====
*** 管理 SVM 最多可以有四個主要金鑰伺服器，但您只需要其中一個主要金鑰伺服器的連接埠號碼和位址，即可在稍後的程序中還原金鑰管理組態。
*** 如果您選擇的主要金鑰伺服器有相關聯的使用者名稱和密碼，則在還原過程中也需要這些資訊。
...
----
.. 如果您執行的是 ONTAP 9.16.1 或更高版本，請取得管理 SVM 的金鑰庫 UUID。
+
...
cluster_B::*
----
.. 請將收集到的資訊保存在安全的地方，以便在後續操作中使用。您現在應該已經收集到以下資訊：
+
*** 管理 SVM 的其中一個主要金鑰伺服器的連接埠號碼和位址
*** 如果選定的主要金鑰伺服器關聯了使用者名稱和密碼，則需要提供這兩項資訊。
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#remove-metrocluster-configurations["移除MetroCluster 部分組態"] 在_MetroCluster Tiebreaker 安裝與設定內容_
...
image::../media/mcc_upgrade_cluster_a_in_switchover_A900.png[MetroCluster切換後，Site_B 處於非活動狀態並準備升級]
...
+
image::../media/drw_9500_remove_PCM.png[從機殼中移除控制器模組：解鎖並滑出]
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png[從機殼移除NVRAM模組：解鎖並拉出標籤]
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png[將NVRAM模組安裝到機殼中：對齊並鎖定到位]
...
+
image::../media/drw_9500_remove_PCM.png[將控制器模組安裝到機殼中：對齊、安裝和連接電纜]
...
** link:../install-ip/task_switch_config_cisco.html#download-and-installing-the-cisco-ip-rcf-files["下載並安裝 Cisco IP RCFs"]
...
NOTE: 執行開機功能表選項時 `6`，MetroCluster bootarg 值可能需要重設。請參閱link:task_upgrade_A700_to_A900_in_a_four_node_mcc_ip_us_switchover_and_switchback.html#set-the-metrocluster-ip-bootarg-variables["設定MetroCluster 靜態IP bootarg變數"]。
...
image::../media/mcc_upgrade_cluster_a_switchback_A900.png[MetroCluster切換後，Site_A 等待升級]
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["新增MetroCluster 功能"] 在_MetroCluster Tiebreaker 安裝和配置_部分。
