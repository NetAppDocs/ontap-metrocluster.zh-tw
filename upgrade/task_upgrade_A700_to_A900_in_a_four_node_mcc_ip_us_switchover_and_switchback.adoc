...
---
= 使用交換器和切換功能、將控制器從AFF 功能性的2700升級至AFF 功能性的S還原MetroCluster IP組態（ONTAP 版本號：32、10.1及更新版本）
:allow-uri-read: 
...
.關於這項工作
* 控制器必須執行ONTAP 的是版本不含更新版本的版本。
* 此程序適用於AFF 在AFF 整個過程中升級至以MetroCluster 32位元IP組態的32位元A700控制器模組。
+
|===
...
+
在MetroCluster 本維護活動之外、不支援以AFF 不支援使用支援的方式操作含有此功能的Ar21、A700和AFF 一個支援的支援功能。
...
在一個VMware上的VMware E3D組態使用DR卡上位於插槽5和7的每個連接埠之一。MetroCluster AFF開始升級之前、如果AFF 在插槽7中有插卡、您必須將其移至叢集所有節點的其他插槽。



=== 在升級控制器之前、請先更新MetroCluster RCF交換器檔案

不使用VLAN、而在整個過程中、使用的是不支援的支援。MetroCluster AFF在不支援VLAN的情況下、使用支援的支援功能可在不使用VLAN的情況下使用。MetroCluster AFF因此、當您將RCF檔案從AFF 某個版本升級至AFF 某個版本時、必須變更RCF檔案。
...
從AFF E不到A700升級到AFF E不到A900時、您不會變更資料網路連接埠、FCP SAN介面卡連接埠、以及SAS和NVMe儲存連接埠。資料生命期會在升級期間和升級後持續運作。因此、您不需要將舊節點的網路連接埠對應至新節點。
...
附註：AFF 此功能可讓您在系統的插槽6中找到更高的介面、而非系統中的其他模組。
...
. 如果您在AFF 使用附加模組做為ESIEA700軟體上的核心傾印裝置、請勿將其傳輸至AFF ESI填寫A900軟體。請勿將AFF 任何零件從顯示器A700控制器模組和NVS移轉至AFF 顯示器A900。




== 安裝AFF 《不需再安裝的顯示器」和「控制器」模組

您必須在AFF 站台B的兩個節點上、安裝升級套件中所提供的支援功能請勿將coredump裝置從AFF ESIEA700 NVS模組移至AFF ESIA900 NVS模組。
...
NOTE: 叢集所有節點上的插槽7應為空白、如所述 <<Map ports from the old nodes to the new nodes>> 區段。
...
在交換AFF 完SESVA900控制器模組和軟體資源之後、您需要將AFF 該節點的叢集開機、並安裝與ONTAP 叢集上執行相同的版本資訊更新和修補程式層級。「netboot」一詞是指您從ONTAP 儲存在遠端伺服器上的不實映像進行開機。準備網路開機時、您必須在ONTAP 系統可以存取的網路伺服器上新增一份《支援》9開機映像。除非安裝在機箱中並已開啟電源、否則無法檢查ONTAP 安裝在AFF 架構控制器模組之開機媒體上的版本。在進行升級的32位元支援系統上執行的支援功能必須與在進行升級的支援功能系統上執行的支援功能相同、而且主要和備份開機映像都必須相符。ONTAP AFF ONTAP AFF您可以在開機功能表中執行netboot、然後執行wappeconfig命令來設定映像。如果控制器模組先前曾在另一個叢集中使用、則「wifeconfig」命令會清除開機媒體上的任何剩餘組態。
...
. 請等待節點B_1現在在AFF 執行於更新控制器模組的情況下開機、並顯示開機功能表選項、如下所示：
+
[listing]
----
