...
---
= 使用切換和切換功能（AFF 例如、更新版本的版本）、將控制器從S還原A700/A900升級至FAS9x00 MetroCluster （以供選擇）ONTAP
:allow-uri-read: 
...
在不使用VLAN的情況下MetroCluster 、無法在AFF 不支援使用VLAN的情況下使用支援。在不支援虛擬區域網路的情況下、使用虛擬區域網路（VLAN）的架構。MetroCluster AFF因此、當您將RCF檔案從AFF 某個版本升級至AFF 某個版本的時、您需要變更RCF檔案、例如從某個版本的E或FAS9000升級至某個版本的E或FAS9500。
