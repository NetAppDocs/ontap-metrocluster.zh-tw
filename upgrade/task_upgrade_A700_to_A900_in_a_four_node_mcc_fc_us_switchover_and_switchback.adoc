...
. 重複更新Fabric B交換器（交換器2和4） ,步驟2 至 ,步驟5.。
...
|===
. 在節點1上執行「netboot」：「netboot http://<web_server_ip/path_to_web_accessible_directory>/netboot/kernel`[]「<path_to_the_web-易於 存取的目錄>」應該會引導您下載「<ONTAP_VERSION >_image.tgz」 ,步驟2。
+
