...
---
= 重新整理四節點或八節點 MetroCluster IP 組態
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 下表列出了所有使用共享的 MetroCluster/HA 連接埠和共享的叢集/HA 連接埠的平台型號。並非所有組合都受此程序支援。請參閱 link:concept_choosing_tech_refresh_mcc.html#supported-metrocluster-ip-tech-refresh-combinations["支援的平台組合"] 以確認您的重新整理組合是否受支援。
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["新增MetroCluster 功能"] 在_MetroCluster決勝規則安裝和配置_中。
