...
* 註 4：整合系統（磁碟和控制器在同一機箱內）的升級需要更換控制器模組，同時保留現有的機箱和磁碟。
...
image:../media/manual-upgrade-combination-group-2-a90-fas90-updated.png["顯示群組 2 平台支援的升級組合"]
...
image::../media/metrocluster_fc_upgrade_table_aff_fas.png[MetroCluster FC 控制器升級支援的平台組合]
