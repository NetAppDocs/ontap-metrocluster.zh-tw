...
+
請依照 link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["設定MetroCluster 靜態IP bootarg變數"] 中的步驟操作。

. 確認 partner-sysid 是否正確：
+
《prontenv合作夥伴sysid》
...
NOTE: 執行開機功能表選項時 `6`，MetroCluster bootarg 值可能需要重設。請參閱link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["設定MetroCluster 靜態IP bootarg變數"]。
...
.. 叢集間 LIF 啟動後、請檢查叢集對等狀態、並視需要重新建立叢集對等關係。
+
您可能需要重新設定叢集對等關係。
