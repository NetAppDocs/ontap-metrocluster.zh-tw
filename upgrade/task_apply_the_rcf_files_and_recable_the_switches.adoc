...
[role="lead"]
您必須套用參考組態檔 (RCF) 來重新設定分區，以容納新節點。

如果無法使用 RCF 設定交換器，則必須手動設定交換器。請參閱：
...
. 找到您組態的 RCF。
+
您必須使用與您的交換器型號相符的八節點組態的 RCF。

. 請按照下載頁面上的說明套用 RCF，並根據需要調整 ISL 設定。
. 確定交換器組態已儲存。
. 重新啟動FC交換器。
. 使用您先前建立的佈線配置、將既有的FC橋接器和新的FC對SAS橋接器連接至FC交換器。
+
FC 交換器連接埠的使用方式必須符合 link:../install-fc/index.html["Fabric附加MetroCluster 的安裝與組態"] 中所述的 MetroCluster 八節點使用方式，才能使用 RCF。
