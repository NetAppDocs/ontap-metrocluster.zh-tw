...
此程序支援 FAS8200 至 FAS9500 、以及 AFF A300 至 AFF A900 控制器升級。
...
在預先檢查開始之前、如果已安裝 ONTAP Mediator 、系統會自動偵測並移除它。若要確認移除、系統會提示您輸入使用者名稱和密碼。當您完成升級、或預先檢查失敗、或您選擇不繼續升級時、您必須執行 link:task_upgrade_controllers_system_control_commands_in_a_four_node_mcc_ip.html#reconfigure-ontap-mediator["手動重新設定 ONTAP Mediator"]。
...
. 在機架或機櫃中安裝控制器模組。
+
https://docs.netapp.com/us-en/ontap-systems/index.html["VMware及其技術資料中心AFF FAS"^]
