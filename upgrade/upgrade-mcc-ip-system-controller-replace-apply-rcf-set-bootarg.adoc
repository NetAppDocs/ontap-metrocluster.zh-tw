...
summary: 為新平台更新交換器 RCFs ，並在控制器模組上設定 MetroCluster IP bootarg 值。 
---
...
.關於這項工作
* 您需要在中的升級程序稍早識別的 UUID 和系統 ID link:upgrade-mcc-ip-system-controller-replace-prechecks.html#gather-information-before-the-upgrade["在升級之前收集資訊"]。
...
.接下來呢？
link:upgrade-mcc-ip-system-controller-replace-reassign-root-agg.html["重新分配根 Aggregate 磁碟"]。
