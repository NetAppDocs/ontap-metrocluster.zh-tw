...
NOTE: 您在過渡和系統重新整理工作流程中會用到這些步驟。您可能已經在這些工作流程中完成了此程序中的某些步驟。
...
NOTE: 僅支援在支援的版本僅在支援的版本僅有支援使用「MetroCluster 支援」ONTAP 功能的指令。
...
+
在 MetroCluster FC 配置中，請轉到 <<fc_step_7_remove_nodes,移除舊的 DR 群組節點>> 的步驟。
...
.. 移除每個待刪除的 DR 群組節點上 pool1 磁碟的所有權。對每個待刪除的節點執行這些步驟。在本步驟的範例中，待刪除的是 node_A_1。
+
... 轉到 nodeshell：
+
`run -node <node_name>`
...
+
所有磁碟均會顯示，包括節點擁有的 Pool1 磁碟。以下範例顯示了部分輸出，其中磁碟 0n.4 及其分割區已指派給 Pool1。輸出結果會因您的配置而異。
...
... 移除 pool1 中每個磁碟的所有權：
+
如果您的組態包含分割磁碟，您必須先移除每個分割磁碟的所有權，然後再移除容器磁碟所有權：

+
`disk remove_ownership <disk_name>`

+
在下列範例中，分割磁碟的所有權會在容器磁碟 `0n.4`的所有權被移除之前移除：
...
. [[fc_step_7_remove_nodes]] 刪除舊 DR 群組中的節點。
+
在每個集群上執行此步驟。
...
.相關資訊
深入瞭解此程序中使用的命令，請參閱 link:https://docs.netapp.com/us-en/ontap-cli/["ONTAP 指令參考"^]。
