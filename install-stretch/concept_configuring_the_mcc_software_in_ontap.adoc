...
image::../media/workflow_high_level_node_and_cluster_configuration_software.gif[ONTAP中的節點和叢集設定工作流程圖]
...
有關設定 ONTAP 的更多一般資訊，請參閱link:https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-ssg/home.html["設定ONTAP"^]
...
* 磁碟機由特定節點擁有；當您建立Aggregate時、該Aggregate中的所有磁碟機都必須由同一個節點擁有、此節點會成為該Aggregate的主節點。
* Aggregate名稱應符合您在規劃MetroCluster 時所決定的名稱配置。
* 其中link:https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-psmg/home.html["磁碟與集合體管理"^]包含有關鏡射彙總的更多資訊。
...
+
link:https://docs.netapp.com/ontap-9/topic/com.netapp.doc.dot-cm-sag/home.html["使用CLI進行叢集管理"^]在「管理組態備份」部分下包含更多資訊。
