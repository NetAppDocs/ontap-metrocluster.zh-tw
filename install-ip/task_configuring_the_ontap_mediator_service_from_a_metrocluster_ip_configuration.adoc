...
.關於這項工作
* 此工作預設會啟用自動非計畫性切換。
* 此任務可在 MetroCluster IP 配置中任何節點的 ONTAP 介面上執行。
* 從 ONTAP 9.18.1 和 ONTAP Mediator 1.11 開始，單一 ONTAP Mediator 執行個體最多可以同時管理十個 MetroCluster 組態。在早期版本中，ONTAP Mediator 最多可以同時支援五個 MetroCluster 組態。
...
. （選用）設定 MetroCluster 自動強制切換。
+
您只能在進階權限層級使用下列命令。

+

IMPORTANT: 使用此指令前，請先檢閱 link:concept-ontap-mediator-supports-automatic-unplanned-switchover.html#mauso-9-12-1["使用MetroCluster 不全自動強制切換的風險與限制"]。
