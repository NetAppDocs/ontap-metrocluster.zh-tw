...
如需使用FabricPools的一般資訊、請參閱 https://["磁碟與層（Aggregate）管理"^]。
...
NOTE: 在設定Aggregate之前、您必須依照FabricPool 中的「設定物件存放區以MetroCluster 供使用的物件組態」所述、設定物件存放區 https://["磁碟與Aggregate管理"^]。
...
. 如FabricPool 所述、使用Aggregate建立鏡射 https://["磁碟與Aggregate管理"^]
+
.. 附加主要物件存放區。
+
此物件存放區實際上較接近叢集。
...
請參閱中的「複寫SVM組態」 http://["資料保護"^] 如需設定SVM DR關係的詳細資訊、
