...
== ONTAP Mediator 的防火牆需求

ONTAP Mediator 使用多個連接埠與特定服務進行通訊。

如果您使用的是第三方防火牆：

* 必須啟用 HTTPS 存取。
* 必須將其設定為允許存取連接埠 31784 和 3260。
+
使用預設的 Red Hat 或 CentOS 防火牆時，防火牆會在 Mediator 安裝期間自動設定。
...
[NOTE]
====
* iSCSI 連接埠僅在 MetroCluster IP 組態中是必要的。
* 22/tcp 連接埠在正常運作中不是必需的，但您可以暫時啟用它進行維護，並在維護工作階段結束後停用它。
