...
---
= 設定Cisco IP 交換器以實現叢集互連和後端MetroCluster IP 連接
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 您必須使用序列主控台連線至交換器。
* 此工作會重設管理網路的組態。
* 如果您將這些步驟作為其他流程（例如交換器更換或升級）的一部分，請檢查是否需要將叢集 LIF 移至合作夥伴交換器，以實現無中斷叢集操作。link:task_install_and_cable_the_mcc_components.html["了解如何設定 MetroCluster IP 交換器"].
...
== 下載並安裝 Cisco 交換器 NX-OS 軟體

您必須將交換器作業系統檔案和RCF檔案下載到MetroCluster 各個交換器、使其採用EWIP組態。

.關於這項工作
此工作需要使用檔案傳輸軟體（例如 FTP、TFTP、SFTP、HTTP 或 SCP）將檔案複製到交換器。

* 如果您將這些步驟作為其他流程（例如交換器更換或升級）的一部分，請檢查是否需要將叢集 LIF 移至合作夥伴交換器，以實現無中斷叢集操作。link:task_install_and_cable_the_mcc_components.html["了解如何設定 MetroCluster IP 交換器"].
* 您必須使用受支援的交換器軟體版本。請參閱 https://hwu.netapp.com["NetApp Hardware Universe"]。
...
. 在 MetroCluster IP 組態中，對剩餘的三個 IP 交換器重複上述步驟。
+

NOTE: 您必須依照下列順序更新交換器：Switch_A_1、Switch_B_1、Switch_A_2、Switch_B_2。如果您的流程需要移動叢集 LIF，則在設定完第一個架構（Switch_A_1、Switch_B_1）中的交換器後，必須重複上述步驟，將叢集 LIF 移至第二個架構（Switch_A_2、Switch_B_2）。請參閱 link:task_install_and_cable_the_mcc_components.html["了解如何設定 MetroCluster IP 交換器"]。
...
CAUTION: 必須確保叢集 LIF 已啟用自動還原功能。

. 如果未啟用自動回復功能，請修改所有叢集 LIF 以啟用自動回復功能：
+

NOTE: 如果您正在執行這些步驟作為其他程序（例如交換器更換或升級）的一部分，則您可能已在這些步驟中停用了自動恢復功能，以完成 link:switch-move-cluster-lifs.html["移動叢集 LIF"]。
