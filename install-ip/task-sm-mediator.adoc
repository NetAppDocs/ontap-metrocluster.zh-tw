...
NOTE: 您只能透過 CLI 設定使用 HTTPS 的 ONTAP Mediator。不支援 System Manager。如需詳細資訊，請參閱link:concept_mediator_requirements.html#select-a-communication-protocol-for-ontap-mediator["選擇 ONTAP Mediator 的通訊協定"]。
