...
|===
+
** 方法1：  to upgrade EFOS with downloading the software image to the backup boot partition
** 方法2：  to upgrade EFOS using the ONIE OS installation
...
NOTE: 在極少數情況下、交換器可能無法開機。請依照  to upgrade EFOS using the ONIE OS installation 以安裝新映像。
...
+
.. 設定交換器基本設定。請遵循步驟4及更新版本：  the Broadcom IP switch to factory defaults
.. 建立並套用RCF檔案、如所述  and installing the Broadcom RCF files
