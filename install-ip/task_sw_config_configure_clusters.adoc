...
image::../media/mcc_dr_groups_4_node.gif[四節點配置中的MetroCluster DR 群組]
...
NOTE: 建立 MetroCluster IP 介面之後、 MetroCluster IP 和連接的交換器連接埠才會上線。
...
* MetroCluster IP 介面使用的實體連接埠取決於平台模式。如需系統的連接埠使用情形、請參閱 link:../install-ip/using_rcf_generator.html["連接MetroCluster 至靜態IP交換器"] 。
* 範例中使用下列IP位址和子網路：
+
|===
...
+
AFF A220 系統的連接埠、顯示如何在支援的平台上使用 `-vlan-id` 參數。

+
針對您的平台機型、在正確的連接埠上設定介面。
...
+
`disk-id` 必須在的遠端機櫃上識別磁碟機 `owner_node_name`。
...
+
 `disk-id`必須在的遠端機櫃上識別磁碟 `owner_node_name`。
