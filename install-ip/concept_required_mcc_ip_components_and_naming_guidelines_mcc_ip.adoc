...
[role="lead"]
確定MetroCluster IP 配置所需的和支援的硬體和軟體組件。查看文件範例中組件的命名約定。
...
*注意：* 25 GbE 速度需要 MAM1Q00A-QSA28 收發器轉接器。
...
NOTE: Aggregate 名稱在所有 MetroCluster 站台上必須是唯一的。這意味著站台 A 和站台 B 上不能有兩個名稱相同的 Aggregate 。
