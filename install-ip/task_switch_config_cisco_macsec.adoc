...
summary: 如有需要，僅在站點間執行的 WAN ISL 連接埠上配置 MACsec 加密。如果選擇使用 MACsec 加密，請在套用正確的 RCF 檔案後進行設定。 
---
= 在 MetroCluster IP 站點中設定 Cisco 9336C 交換器的 MACsec 加密（選用）
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如有需要，您可以在站點間執行的 WAN ISL 連接埠上設定 MACsec 加密。如果您選擇使用 MACsec 加密，則必須在套用正確的 RCF 檔案後進行設定。

[NOTE]
====
* MACsec 加密只能套用於站台之間執行的 WAN ISL 連接埠。
* 設定 Cisco 9336C 交換器時，設定 MACsec 加密為選用項目。
