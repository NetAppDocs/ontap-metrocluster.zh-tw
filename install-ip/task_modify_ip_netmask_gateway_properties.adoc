...
---
= 修改MetroCluster 位址、網路遮罩和閘道、使其位於一個靜態IP中
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
從ONTAP 《E59.10.1》開始、您可以變更MetroCluster 下列的EIP介面屬性：IP位址和遮罩、以及閘道。您可以使用任何參數組合來更新。
...
使用「MetroCluster 畫面組態設定介面show」、「MetroCluster 畫面連線」檢查和「MetroCluster 畫面顯示」命令、確認所有連線功能都能在所有介面上運作。
...
----
. [[step3]確認所有連線都正常運作：MetroCluster 「顯示組態設定連線」
