...
* 使用 Cisco 9336C-FX2 交換器連接 NS224 儲存設備時，最多只能設定兩個 MetroCluster 組態或 DR 群組。至少一個 MetroCluster 組態或 DR 群組需要將 NS224 機架連接到 MetroCluster 交換器。如果其中一個 MetroCluster 組態或 DR 群組連接的系統不支援 NS224 機架，則只能將其作為第二個 MetroCluster 組態或 DR 群組連接。
+
