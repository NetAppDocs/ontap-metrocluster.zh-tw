...
---
= 在 MetroCluster IP 組態中，對 Cisco 交換器進行無中斷的 ISL 連接埠新增、移除或變更
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: 如果您在共享的組態中使用 Cisco 9336C-FX2 交換器連接 NS224 機架，變更 ISL 可能需要新的參考組態檔案 （RCF）。如果目前和新的 ISL 速度分別為 40 Gbps 和 100 Gbps，則無需新的 RCF。所有其他 ISL 速度變更都需要新的 RCF。例如，將 ISL 速度從 40 Gbps 變更為 100 Gbps 不需要新的 RCF，但將 ISL 速度從 10 Gbps 變更為 40 Gbps 則需要新的 RCF。
...
NOTE: 如果您在共享的組態中使用 Cisco 9336C-FX2 交換器連接 NS224 機架，並且需要升級 RCF 並為新的 ISL 連接埠套用新組態，請依照下列步驟操作：link:task_upgrade_rcf_files_on_mcc_ip_switches.html["升級 MetroCluster IP 交換器上的 RCF。"]
