...
[role="lead"]
您可能需要升級 MetroCluster IP 交換器上的參考組態檔案（RCF）。例如，如果交換器上執行的 RCF 版本與 ONTAP 版本、交換器韌體版本或兩者都不相容。

.開始之前
* 如果您要安裝新的交換器韌體，則必須先安裝交換器韌體，然後再升級 RCF。
...
.關於這項工作
* 此程序會中斷正在升級 RCF 的交換器上的流量。套用新的 RCF 後，流量將恢復。
...
. 根據您的交換器廠商，下載並安裝 IP RCF。
+
** link:../install-ip/task_switch_config_broadcom.html#downloading-and-installing-the-broadcom-rcf-files["下載並安裝 Broadcom IP RCF"]
...
NOTE: 在第一個架構（Switch_A_1、Switch_B_1）中設定交換器後，必須重複上述步驟，將叢集 LIF 移轉到第二個架構（Switch_A_2、Switch_B_2）。請參閱 link:../install-ip/task_install_and_cable_the_mcc_components.html["了解如何設定 MetroCluster IP 交換器"]。
