...
.步驟
. 使用 https://["Fabric附加MetroCluster 的安裝與組態"^] 若要判斷交換器類型的纜線配置、請使用連接埠使用方式進行八節點MetroCluster 的邊邊架構。
+
...
FC交換器連接埠使用量必須符合中所述的使用量 https://["Fabric附加MetroCluster 的安裝與組態"^] 以便能夠使用RCT。
...
. 請使用中所述的協調切換、修復及切換程序 https://["支援管理與災難恢復MetroCluster"^]。
