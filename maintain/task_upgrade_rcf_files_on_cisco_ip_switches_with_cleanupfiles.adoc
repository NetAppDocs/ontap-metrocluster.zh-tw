...
. 將檔案下載至bootflash。
+
.. 下載您在中建立的高效能檔案 ,建立RCF檔案和CleanUpFiles、或為目前組態建立一般的CleanUpFiles
+
...
.. 下載您在中建立的「新」RCF檔案 ,建立新組態的「新」RCF檔案。
+
Switch-A1的RCF檔案範例：「NX9336_v1.90_Switch-a1.txt'

.. 下載您在中建立的高效能檔案 ,建立新組態的「新」RCF檔案。 此步驟為選用步驟、您日後可以使用此檔案來更新交換器組態。它符合目前套用的組態。
+
