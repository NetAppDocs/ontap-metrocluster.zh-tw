...
---
= 在 MetroCluster IP 組態中更換或重新規劃交換器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
CAUTION: 如果您需要在 AFF A220、AFF A250 或 FAS2750 平台上執行交換器更換，請參閱link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/Cluster_and_HA_communication_issues_during_back-end_switch_replacement["知識庫文章 "後端交換器更換期間的叢集和 HA 通訊問題""^]。
...
[NOTE]
====
使用 X60134A 網路介面卡的平台不支援混合連結速度。若要檢查您的平台是否受到影響，請參閱 https://hwu.netapp.com/["Hardware Universe"^]。

如果您在受影響的平台上以使用 100G 連接埠的新交換器取代使用 25G 連接埠的交換器（例如，在 AFF A50 平台上以 Cisco 9336C 交換器取代 BES-53248 交換器），請參閱 link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/100G_link_not_coming_online_during_MetroCluster_back-end_switch_replacement?caseId=2010523490["知識庫文章 "100G 連結在 MetroCluster 後端交換器更換期間無法線上""^]。
...
NOTE: 您必須依照下列順序更新交換器：Switch_A_1、Switch_B_1、Switch_A_2、Switch_B_2。在第一個架構（Switch_A_1、Switch_B_1）中設定交換器後，您必須重複上述步驟，將叢集 LIF 移轉到第二個架構（Switch_A_2、Switch_B_2）。請參閱 link:../install-ip/task_install_and_cable_the_mcc_components.html["了解如何設定 MetroCluster IP 交換器"]。
...
. 連接交換器纜線。
+
您可以按照以下步驟操作link:../install-ip/using_rcf_generator.html["連接MetroCluster 至靜態IP交換器"]。
...
.. 如果本機叢集介面 * 未 * 連線至交換器：
+
... 請依照 link:https://docs.netapp.com/us-en/ontap-systems-switches/switch-bes-53248/migrate-to-2n-switched.html["移轉至交換式NetApp叢集環境"^] 程序將無交換器式叢集轉換為有交換器叢集。使用 link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/using_rcf_generator.html["安裝與組態MetroCluster"] 或 RCF 佈線檔案中指示的連接埠連接本機叢集介面。
...
. 開啟交換器或交換器的電源。
+
如果新交換器與舊交換器相同，則啟動新交換器。如果要升級或降級交換器，則啟動兩台交換器。在第二個架構更新之前，每個站台可以使用兩台不同的交換器進行組態。

. 重複、確認 MetroCluster 組態正常 <<all_step1,步驟1.>>。
+
如果您正在升級或降級第一個架構中的交換器，您可能會看到一些與本機叢集相關的警示。

+

NOTE: 如果升級或降級架構，則對第二個架構重複所有步驟。

. 將所有叢集 LIF 回復到其主連接埠。
+

CAUTION: 在完成所有需要更新的交換器的先前步驟之前，請勿將叢集 LIF 回復至其主連接埠。
