...
----
. 列出 ONTAP 中包含介面的 SVM 的 iSCSI 介面： `iscsi interface show -vserver svm-name`
+
[listing]
----
...
. 在主機上、發出下列命令以重新掃描主機上的 SCSI 匯流排、並探索新增的路徑： `/usr/bin/rescan-scsi-bus.sh -a`
+
[listing]
----
...
----
. 確認 `sg3-utils` 套件安裝在 Linux 主機上。如此可避免發生 `rescan-scsi-bus.sh utility not found` 使用重新掃描 Linux 主機以取得新對應的 LUN 時發生錯誤 `rescan-scsi-bus` 命令。
. 在 iSCSI 主機上、重新掃描 SCSI 匯流排： `/usr/bin/rescan-scsi-bus.sh -r`
+
移除的路徑是FC節點的路徑。
...
.
----
. 在 iSCSI 主機上、確認只能看到來自 MetroCluster IP 節點的路徑：
+
`sanlun lun show -p`
