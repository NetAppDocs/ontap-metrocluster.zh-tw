...
* MetroCluster FC 節點稱為舊節點、 MetroCluster IP 節點稱為新節點。
...
LUN 和磁碟區仍然實體託管在舊節點上。由於 LUN 僅在舊節點介面上報告，因此主機僅顯示經過舊節點的路徑。若要查看此資訊，請在主機上執行 `sanlun lun show -p`和 `multipath -ll -d`命令並檢查命令輸出。
