...
橋接CLI命令（例如「串橋路執行CLI -namebridge-name-commandbridge命令-command-name'命令）可從ONTAP Isfit介面執行。
...
. 如有需要、請停用服務、方法是發出「et closePort _service_」、並視需要重複執行命令、直到停用所有所需的服務為止：
+
「et closePort _service_」

+
「設為關閉連接埠」命令一次會停用單一服務。此服務可由下列其中一項指定：
...
+
「et ipdhcp MP1 disabled'set ethernetspeed MP1 1000」
...
. 如果使用CLI、您必須執行下列命令：
+
「et FCDataRate port-numberport-speed」
...
[NOTE]
====
設定FIBreBridge 7600N橋接器時、不需要FCConnMode設定。如果您使用CLI、則必須執行下列命令：

「et FCConnMode連接埠編號PTP」
...
+
預設會啟用SAS連接埠A到D。您必須停用未使用的SAS連接埠。
...
. 保護橋接器的存取安全、並儲存橋接器的組態。根據ONTAP 系統執行的版本、從下列選項中選擇一個選項。
+
[cols="1,3"]
|===
...
.步驟
. [step1_verify_bridges]確認每個橋接器都能偵測到其所連接的所有磁碟機和磁碟櫃：
+
[cols="1,3"]
|===
| 如果您使用... | 然後... 
...
.. 按一下*進階*。
.. 使用「sastarges'」命令檢視連線的裝置、然後按一下「*提交*」。
...
NOTE: 如果輸出開頭出現截短的文字回應、您可以使用Telnet連線到橋接器、然後輸入相同的命令來查看所有輸出。
...
正確
 a| 
重複 <<step1_verify_bridges,步驟1.>> 每個剩餘的橋接器。
...
+
link:task_fb_new_install.html["將磁碟櫃連接至橋接器"]

.. 重複 <<step1_verify_bridges,步驟1.>>。
...
* 可使用「unsecurebridge」命令重新啟用乙太網路連接埠。
...
+
「torage bridge run－CLI bridge bridge-name－command saveconfigur態」
...
+
「torage bridge run－CLI bridge bridge bridge bridge－name－command firmwarerestart」
