...
== FiberBridge 7600N 和 7500N 橋接器上支援的磁碟和磁碟櫃數量

下表顯示一對 FiberBridge 7600N 和 7500N 橋接器支援的磁碟和機櫃數量。

[NOTE]
====
* 您達到的第一個限制適用。
* 如果您使用的是 DS460C 機櫃、最多只能使用 240 個磁碟機、相當於 4 個機櫃。
* 如果您使用的是 DS212C 機櫃、最多只能使用 120 個磁碟機、相當於 10 個機櫃。


====
.支援的磁碟數量
檢閱一對 FiberBridge 7600N 或 7500N 橋接器支援的磁碟數量：

[cols="3*"]
|===
| 磁碟組 | 磁碟類型 | 每個橋接器配對的磁碟總數 
...
.2+| 混合（ 96 個 SSD ） | SSD | 96 


| HDD | 144 


.2+| 混合式（ 72 個 SSD ） | SSD | 72. 


| HDD | 168 


.2+| 混合式（ 48 個 SSD ） | SSD | 48 


| HDD | 192 


.2+| 混合式（ 24 個 SSD ） | SSD | 24 


| HDD | 216 
|===
.支援的機櫃數量
檢閱一對 FiberBridge 7600N 和 7500N 橋接器背後支援的機櫃數量：
