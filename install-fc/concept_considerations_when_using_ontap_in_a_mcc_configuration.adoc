...
NOTE: 在設定Aggregate之前、您必須依照FabricPool 中的「設定物件存放區以MetroCluster 供使用的物件組態」所述、設定物件存放區https://["磁碟與集合體管理"]
...
. 如FabricPool 中所述、使用Aggregate建立鏡射 https://["磁碟與集合體管理"]
+
.. 附加主要物件存放區。
+
此物件存放區實際上較接近叢集。
...
從ONTAP 功能性的9.11.1開始、 https://["一致性群組"^] 支援的功能包括MetroCluster ：
