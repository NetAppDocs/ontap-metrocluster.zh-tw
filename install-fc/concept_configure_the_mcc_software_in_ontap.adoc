...
如果您需要以網路方式開機新的控制器、請參閱 http://["Netbooting新的控制器模組"] 請參閱《_ MetroCluster 升級、轉換與擴充指南》_。
...
NOTE: 最佳實務做法是擁有多個資料集合體。如果第一個DR群組只有一個Aggregate、而您想要新增一個具有單一Aggregate的DR群組、則必須將中繼資料磁碟區從單一資料Aggregate中移出。如需此程序的詳細資訊、請參閱 http://["在MetroCluster 不完整的組態中移動中繼資料磁碟區"]。
...
.步驟
. 請使用中所述的協調切換、修復及切換程序 https://["從災難中恢復"]。
