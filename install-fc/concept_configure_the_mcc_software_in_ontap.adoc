...
summary: 您必須在MetroCluster 不完整的功能組態中設定每個節點ONTAP 、包括節點層級組態、以及將節點組態設定為兩個站台。 
---
= 在 ONTAP 中設定 MetroCluster FC 軟體
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您必須在 ONTAP 中設定 MetroCluster FC 組態中的每個節點，包括節點層級組態以及將節點組態到兩個站台。您還必須實作兩個站台之間的 MetroCluster 關係。

image::../media/workflow_high_level_node_and_cluster_configuration_software.gif[高階節點和叢集配置的工作流程]
...
在 MetroCluster FC 組態中設定儲存系統時、您必須確定控制器模組和機箱元件的高可用度（ HA ）狀態為 MCC 或 MCC-2n 、以便這些元件能正常開機。雖然此值應預先設定在從原廠收到的系統上、但仍應在繼續之前確認設定。
...
如需設定 ONTAP 的一般資訊、請參閱 link:https://docs.netapp.com/us-en/ontap/task_configure_ontap.html["設定ONTAP 功能"^]。
...
.相關資訊
link:https://docs.netapp.com/us-en/ontap/volumes/index.html["使用CLI進行邏輯儲存管理"^]
...
* Aggregate名稱應符合您在規劃MetroCluster 時所決定的名稱配置。請參閱 link:https://docs.netapp.com/us-en/ontap/disks-aggregates/index.html["磁碟與Aggregate管理"^]。
...
.相關資訊
link:https://docs.netapp.com/us-en/ontap/disks-aggregates/index.html["磁碟與層（Aggregate）管理"^]
...
[[snmpv3-mcc-fc]]
=== 在 MetroCluster FC 設定中設定 SNMPv3

.開始之前
交換器和ONTAP 作業系統上的驗證和隱私權傳輸協定必須相同。

.關於這項工作
ONTAP 目前支援 AES-128 進行隱私保護（加密），支援 SHA-256 進行驗證。

[CAUTION]
====
* 在 Fabric OS 10.x 及更高版本中，Brocade 僅支援 SHA-512 進行 SNMP 驗證。ONTAP 目前不支援 SHA-512，僅支援 SHA‑256。
* 如果您的 MetroCluster FC 交換器執行的是 Fabric OS 10.x 或更新版本，則無法設定 SNMP，也無法將這些交換器新增至 ONTAP 健康監控中。如果您想降級交換器韌體，可以依照 link:../maintain/task_upgrade_or_downgrad_the_firmware_on_a_brocade_fc_switch_mcc.html["升級或降級Brocade FC交換器上的韌體"] 中的步驟進行。
...
NOTE: 對於 EngineID，按 *ENTER* 分配預設值。
...
--
.Brocade交換器（FOS 8.x 及更早版本）
--
「nmpconfig - set v3」

此範例顯示如何設定唯讀使用者。如果需要，您可以調整 RW 使用者。如果需要 RO 存取權限，請在「使用者 (ro)：」後面指定「snmpv3user」。
...
NOTE: 您也應該為未使用的帳戶設定密碼、以確保密碼安全、並使用ONTAP 您的版本資訊系統所提供的最佳加密功能。
...
NOTE: 為了提升安全性， NetApp 建議您設定 SNMPv2 或 SNMPv3 來監控交換器健全狀況。

.關於這項工作
這些工作僅適用於具有FC至SAS橋接器的系統。

從 Fabric OS 9.0.1 開始，Brocade 交換器的健康監控不再支援 SNMPv2，您必須改用 SNMPv3。如果您使用 SNMPv3，則必須先在 ONTAP 中設定 SNMPv3，然後再繼續執行下列部分。如需更多詳細資訊，請參閱 <<snmpv3-mcc-fc,在 MetroCluster FC 設定中設定 SNMPv3>>。

[NOTE]
====
* 您應該將橋接器和節點管理LIF放在專用網路中、以避免來自其他來源的干擾。
* 如果您使用專用網路進行健全狀況監控，則每個節點在該專用網路中都必須有節點管理 LIF 。


====
NetApp 僅支援下列工具來監控 MetroCluster FC 組態中的元件：

* Brocade網路顧問（BNA）
* Brocade SANnav
* Active IQ Config Advisor
* NetApp 健全狀況監控（ ONTAP ）
* MetroCluster 資料收集器（ MC_DC ）
...
NOTE: 從 ONTAP 9.8 開始， `storage switch`命令將替換爲 `system switch fibre-channel`。下列步驟顯示 `storage switch`命令，但如果您執行的是 ONTAP 9.8 或更新版本，則建議使用該 `system switch fibre-channel`命令。

.步驟
. 將IP位址的交換器新增至每MetroCluster 個節點：
+
您執行的命令取決於您使用的是 SNMPv2 或 SNMPv3 。
...
.相關資訊
link:https://docs.netapp.com/us-en/ontap/cloud/high-availability-concept.html["高可用度組態"^]
