...
* ,在ONTAP 八模式或四節點MetroCluster 的不全功能組態中設定功能
...
| * ONTAP 在S69.6及更新版本中：* | 「網路介面show -service-policy default-intercluster -容 錯移轉」 


| * ONTAP 在《S19.5及更早版本》中：* | 「網路介面show -role intercluster -容 錯移轉」 
|===
+
如需完整的命令語法、請參閱手冊頁。
...
+
以下範例建立叢集間lifs cluster01_icl01和cluster01_icl02：
...
NOTE: 如果您要使用單一鏡射資料Aggregate、請參閱 ,步驟1. 以取得相關指示。
