...
NOTE: 系統會自動產生您所需的新授權金鑰、並將其傳送至檔案上的電子郵件地址。如果您在30天內未收到附有授權金鑰的電子郵件、請參閱中的_「如果我的授權有問題、該聯絡誰？」一節link:https://kb.netapp.com/Advice_and_Troubleshooting/Flash_Storage/AFF_Series/Post_Motherboard_Replacement_Process_to_update_Licensing_on_a_AFF_FAS_system["NetApp知識庫：更新AFF/FAS系統授權的主機板更換程序。"^]
...
----
+
如果災難站台包含未鏡射的Aggregate、而未鏡射的Aggregate不再存在、則儲存Aggregate show命令的輸出中可能會顯示「'unknown'」狀態。請聯絡技術支援部門、以移除未鏡射集合體的過時項目、請參閱 link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/MetroCluster/How_to_remove_stale_unmirrored_aggregate_entries_in_a_MetroCluster_following_disaster_where_storage_was_lost["NetApp知識庫：如何在MetroCluster 發生儲存設備遺失的災難後、移除過時的未鏡射Aggregate項目。"^]
