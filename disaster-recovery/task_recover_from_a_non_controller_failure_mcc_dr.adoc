...
在執行切換之前，請確認 LIF 託管在適當的節點和連接埠上。

.步驟
. 在執行切換之前，請確認 LIF 已託管在適當的節點和連接埠上。
+
.. 變更為進階權限層級：
+
"進階權限"
...
CAUTION: 在執行回切操作之前，您必須確認新節點上資料 LIF 的位置是否正確。回切組態時，ONTAP 會嘗試恢復 LIF 使用的主連接埠上的流量。如果主連接埠與交換器連接埠和 VLAN 的連線不正確，則可能發生 I/O 故障。如果您在將 LIF 放置在正確的節點或連接埠上時遇到任何問題，請不要繼續執行回切操作，並聯絡 NetApp 技術支援。
