...
---
= 確保 MetroCluster Tiebreaker 主機和資料庫安裝安全
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 使用資料庫加密
+
** 支援靜態資料加密。link:https://mariadb.com/docs/server/security/encryption/data-at-rest-encryption/data-at-rest-encryption-tde-fundamentals["深入瞭解閒置資料加密"^]
...
.接下來呢？
link:install-choose-procedure.html["選擇您的 MetroCluster Tiebreaker 安裝程序"] 以安裝或升級 Tiebreaker。
