...
---
= 使用 MetroCluster Tiebreaker 監控 MetroCluster 組態
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
您可以停用 Tiebreaker 軟體的觀察者模式，使其在偵測到網站故障後自動啟動切換。


NOTE: 在 Tiebreaker 1.6 及更高版本中，您可以使用以下方式啟用新增顯示器時的自動切換： `monitor add wizard`命令。參考link:concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["新增MetroCluster 功能"]。
